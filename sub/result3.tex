...
LAC data and GAC data have been used to create monthly mean chlorophyll-a concentration and its difference from 2003 to 2006 as shown in Figure \ref{fig:diff_2003} - \ref{fig:diff_2006}. The difference between the LAC and the GAC data was less than 0.21 $\rm mg~m^{-3}$ for every month and it was not a small value. It could be inferred that the spatial resolution of a data must be considered more deeply. The maximum difference between the LAC and the GAC data appeared on April, 2004. 
  
 The number of pixels with a value over 10 $\rm mg~m^{-3}$ is also shown in the Figures. It was discovered that LAC data had more pixels over 10 $\rm mg~m^{-3}$, compared to GAC data except on April 2003 and July 2006. The number of pixels did not show a clear tendency. This could mean these data include many speckles or the number of high chlorophyll-a concentration is just not consistent. GAC data had dates which did not have any pixels over 10 $\rm mg~m^{-3}$. It was in May and June, when the chlorophyll-a concentration decreased. 
...
\begin{figure}[h]
	...
	\begin{axis} [
	width=1.00\textwidth,height = 8cm,%
	axis y line*=right,%
	xtick=data,%
	symbolic x coords={Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec},%
	...
	\end{tikzpicture}
	%	\end{pgfonlayer}
	\scriptsize\caption{Variation of monthly mean values of (a) chlorophyll-a concentration in 2003 using the LAC data (blue line) and the GAC data (red line) and its differences between the LAC and the GAC data and (b) the number of pixels with a value over 10 $\rm mg~m^{-3}$.}
	\label{fig:diff_2003}
\end{figure}
...
\begin{figure}[h]
	...
	\end{axis}
	\end{tikzpicture}
	\scriptsize\caption{(a) Variation of monthly mean values of chlorophyll-a concentration in 2004 using the LAC data (blue line) and the GAC data (red line) and its differences between the LAC and the GAC data and (b) the number of pixels with a value over 10 $\rm mg~m^{-3}$.}
	\label{fig:diff_2004}
\end{figure}
...
\begin{figure}[h]
	...
	\end{axis}
	\end{tikzpicture}
	\scriptsize\caption{(a) Variation of monthly mean values of chlorophyll-a concentration in 2005 using the LAC data (blue line) and the GAC data (red line) and its differences between the LAC and the GAC data and (b) the number of pixels with a value over 10 $\rm mg~m^{-3}$.}
	\label{fig:diff_2005}
\end{figure}
...
\begin{figure}[h]
	...
	\end{axis}
	\end{tikzpicture}
	\scriptsize\caption{(a) Variation of monthly mean values of chlorophyll-a concentration in 2006 using the LAC data (blue line) and the GAC data (red line) and its differences between the LAC and the GAC data and (b) the number of pixels with a value over 10 $\rm mg~m^{-3}$.}
	\label{fig:diff_2006}
\end{figure}
